...
const commands = {
  help: (message, user, args) => {
    const channel = `\`${config.irc.clientOpts.channels[0]}\``;
    return `Howdy, ${user.nick}! I'm ${config.flowdock.nick}, your friendly IRC ↔ Flowdock Bot. Here's how I work:

By default, whatever you type into this Flow will be echoed into an IRC channel.
I'm currently configured to echo into ${channel} on \`${config.irc.server}\`.
My nick there is ${config.irc.nick}.
...
addressing your commands to me, like this: \`@${config.flowdock.nick} ping\`. The output from your
...
`;
  ...
  link: (message, user, args) => {
    ...
    case 2:
      ...
      break;
    default:
      response = `You didn't give me an IRC nick to link. Try \`@${config.flowdock.nick} help\` to learn how to use the \`link\` command.`;
    }
    return response;
  },
  ...
  unlink: (message, user, args) => {
    ...
    case 2:
      ...
      break;
    default:
      response = `You didn't give me an IRC nick to unlink. Try \`@${config.flowdock.nick} help\` to learn how to use the \`unlink\` command.`;
    }
    return response;
  }
};
...
const run = (command, message, user, args) => {
  ...
  if (Object.keys(commands).indexOf(command) === -1) {
    return `Sorry, I don't know anything about "${command}". :cry:\nYou can ask me for help if you want: \`@${config.flowdock.nick} help\``;
  }
